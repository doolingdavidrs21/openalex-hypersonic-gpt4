...
st.title("Example: Hypersonic Technologies")
st.markdown("""
...
def load_centroids_asat():
  ...
  #  df = pd.read_json(df.to_json())
    dg = pd.read_pickle('hypersoniccentroids2d.pkl.gz')
    #return dg
    return dg[dg.cluster != -1]

@st.cache_data()
def load_dftriple_asat():
    dg = pd.read_pickle('hypersonicdftriple2d.pkl.gz')
    return dg

@st.cache_data()
def load_dfinfo_asat():
    dg = pd.read_pickle('hypersonicdfinfo2d.pkl.gz')
    #return dg
    return dg[dg['cluster'] != -1]
